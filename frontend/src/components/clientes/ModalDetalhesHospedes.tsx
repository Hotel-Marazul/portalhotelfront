    ...
    <Box>
      ...
      <Box className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Box>
          ...
          <Typography>
            <strong>Automovel:</strong> {cliente.automovel || "-"}
          </Typography>
          <Typography>
...
function HospedesLista({ guests }: { guests: GuestSummaryDto[] }) {
  if (guests.length === 0) {
    return <Typography className="text-gray-500 italic">Nenhum hospede.</Typography>;
  }

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
      {guests.map((hospede) => (
        <Box key={hospede.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100">
          <Typography>
...
function ReservaCard({ reserva, index }: { reserva: ReservationSummaryDto; index: number }) {
  const checkIn = useMemo(() => formatDate(reserva.checkInDate), [reserva.checkInDate]);
  const checkOut = useMemo(() => formatDate(reserva.checkOutDate), [reserva.checkOutDate]);
  const total = useMemo(() => formatCurrencyBRL(reserva.totalPrice), [reserva.totalPrice]);
  const room = reserva.room;
  const guests = Array.isArray(reserva.guests) ? reserva.guests : [];
    ...
    <Card className="border border-gray-200 rounded-xl shadow-sm">
      <CardContent className="space-y-4">
        <Box className="flex justify-between items-center gap-4">
          ...
          <Typography className="text-sm text-gray-600">Total: {total}</Typography>
        </Box>
        ...
        <Box className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Box>
            <Typography>
              <strong>Quarto:</strong> {room?.roomNumber ?? "-"}
            </Typography>
            <Typography>
              <strong>Status do quarto:</strong> {room?.status ?? "-"}
            </Typography>
            <Typography>
              <strong>Categoria:</strong> {room?.categoryName ?? "-"} -{" "}
              {formatCurrencyBRL(room?.categoryPrice)}
            </Typography>
          </Box>
        </Box>
...
export default function ModalDetalhesHospedes({
  ...
  const reservations = Array.isArray(cliente.reservations) ? cliente.reservations : [];
      ...
      <DialogContent className="space-y-6 p-6">
        ...
        <Box>
          ...
          {reservations.length === 0 && (
            <Typography className="text-gray-500 italic">Nenhuma reserva encontrada.</Typography>
          )}

          {reservations.length > 0 && (
            <div className="space-y-4 mt-3">
              {reservations.map((reserva, index) => (
                <ReservaCard key={reserva.id ?? index} reserva={reserva} index={index} />
              ))}
            </div>
          )}
        </Box>
