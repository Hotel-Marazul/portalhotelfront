...
import { createReservationSchema, reservationIdSchema, updateReservationSchema } from "./reservations.schema.js";
...
async function ensureRoomIsAvailable(roomId, checkInDate, checkOutDate, ignoreReservationId) {
    const conflicts = await query(`
      ...
      LIMIT 1
    `, [roomId, checkInDate, checkOutDate, ignoreReservationId ?? null]);
    if (conflicts.length > 0) {
        throw new HttpError(409, "Já existe uma reserva nesse quarto para o período informado.");
    }
}
...
function reservationRowsToDto(reservations, guests) {
    ...
    return reservations.map((reservation) => ({
        ...
        room: reservation.room_number !== null
            ...
            : undefined,
        client: reservation.client_full_name
            ? {
...
async function getReservationsByIds(reservationIds) {
...
reservationsRouter.get("/Reservations", asyncHandler(async (_req, res) => {
    const reservations = await getReservationsByIds();
    res.json(reservations);
}));
reservationsRouter.post("/Reservations", validate({ body: createReservationSchema }), asyncHandler(async (req, res) => {
    ...
    if (room.status === "Manutenção") {
        throw new HttpError(400, "Quarto em manutenção não pode receber reservas.");
    }
    const clients = await query(`SELECT id FROM clients WHERE id = $1 LIMIT 1`, [
        req.body.clientId
    ]);
    if (clients.length === 0) {
    ...
    const pricingRuleRows = await query(`
    ...
    const totalPrice = calculateReservationTotal(Number(room.daily_price), req.body.checkInDate, req.body.checkOutDate, guests, pricingRules);
    ...
    if (isActiveReservationStatus(req.body.status)) {
        await updateRoomOccupancy(req.body.roomId);
    }
    const reservations = await getReservationsByIds([reservationId]);
    res.status(201).json(reservations[0]);
}));
reservationsRouter.put("/Reservations/:id", validate({ params: reservationIdSchema, body: updateReservationSchema }), asyncHandler(async (req, res) => {
    ...
    validateReservationDates(req.body.checkInDate, req.body.checkOutDate);
    await ensureRoomIsAvailable(req.body.roomId, req.body.checkInDate, req.body.checkOutDate, existingReservation.id);
    const pricingRuleRows = await query(`
        SELECT id, name, description, min_age, max_age, price::text AS price
        FROM pricing_rules
      `);
    ...
    await updateRoomOccupancy(req.body.roomId);
    if (existingReservation.room_id !== req.body.roomId) {
        await updateRoomOccupancy(existingReservation.room_id);
    }
    const reservations = await getReservationsByIds([existingReservation.id]);
    res.json(reservations[0]);
}));
reservationsRouter.delete("/Reservations/:id", validate({ params: reservationIdSchema }), asyncHandler(async (req, res) => {
    const existingReservations = await query(`
        SELECT id, room_id
        FROM reservations
        WHERE id = $1
    ...
    await updateRoomOccupancy(reservation.room_id);
    res.status(204).send();
}));
reservationsRouter.get("/reservations/counter-summary", asyncHandler(async (_req, res) => {
    const rows = await query(`
        SELECT
          ...
          COUNT(*) FILTER (WHERE status IN ('Confirmada', 'EmAndamento', 'Pendente'))::int AS reservas_ativas,
          (SELECT COUNT(*)::int FROM rooms WHERE status = 'Ocupado') AS occupied_rooms,
    ...
    const occupancyRate = summary.total_rooms === 0 ? 0 : (summary.occupied_rooms / summary.total_rooms) * 100;
    res.json({
        taxaOcupacaoMes: [
...
reservationsRouter.get("/reservations/revenue-summary", asyncHandler(async (_req, res) => {
    const rows = await query(`
        SELECT
