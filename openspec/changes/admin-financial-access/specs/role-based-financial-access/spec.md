...
- **WHEN** uma sessão válida com papel `manager` consulta o endpoint de sessão
- **THEN** o sistema retorna `role: manager`
...
- **WHEN** um usuário autenticado com papel `manager` consulta o resumo financeiro
...
O sistema SHALL permitir que `manager` consulte preço total, total pago, saldo e lançamentos de uma reserva específica e registre pagamentos permitidos pelas regras operacionais existentes.

#### Scenario: Recepção confere e registra recebimento

- **WHEN** um usuário `manager` abre uma reserva autorizada e registra um pagamento válido
...
- **WHEN** um usuário `manager` tenta registrar um pagamento inválido
...
O frontend SHALL apresentar a mesma rota de dashboard com conteúdo operacional para `admin` e `manager`, acrescentando indicadores financeiros gerenciais somente para `admin`.
...
- **WHEN** a sessão atual possui papel `manager`
